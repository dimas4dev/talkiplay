  ...
  return (
    <div className={`relative ${className}`}>
      <div className="relative grid grid-cols-[120px_24px_1fr] gap-y-6">
        ...
        <span className="pointer-events-none absolute top-2 bottom-2 left-[132px] w-[2px] bg-[var(--color-status-active-bg)]" />
        ...
        {items.map((it, index) => (
          <React.Fragment key={`${it.date}-${index}`}>
            ...
            <div className="relative mt-1 flex items-start justify-center">
              <span className="z-10 inline-block h-3 w-3 rounded-full bg-[var(--color-status-active-bg)]" />
            </div>
            {/* Contenido */}
